...
def main():
    while True:
        ...
        if int(wifi_signal_strength) < WIFI_SIGNAL_THRESHOLD:
            send_email(
                "Weak Wi-Fi signal",
                f"The Wi-Fi signal strength is weak ({wifi_signal_strength}).",
            )
        ...
        if memory_usage > MEMORY_THRESHOLD:
            send_email(
                "High memory usage", f"The memory usage is high ({memory_usage}%)."
            )
        ...
        if temperature > TEMPERATURE_THRESHOLD:
            send_email(
                "High temperature", f"The temperature is high ({temperature}°C)."
            )
