...
class PiNetworkApi:
    ...
    def send_transaction(self, user: User, recipient: str, amount: int) -> bool:
        ...
        if response.status_code == 201:
            ...
            print(
                f"Error sending transaction: {response.status_code} {response.reason}"
            )
            return False
    ...
    def get_transaction_history(self, user: User) -> Optional[List[Dict]]:
        ...
        response = requests.get(
            f'{self.api_endpoint}/wallet/{user.wallet["address"]}/transactions'
        )
        if response.status_code == 200:
            data = response.json()
            ...
            print(
                f"Error getting transaction history: {response.status_code} {response.reason}"
            )
            return None
