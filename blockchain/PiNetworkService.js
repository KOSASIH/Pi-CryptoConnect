...
class PiNetworkService {
  ...
  async getAccountBalance() {
    const account = await this.client.stellar.accounts.account(
      this.client.stellar.accounts.myAccountId(),
    );
  ...
  async sendPi(amount, destination) {
    ...
    const result =
      await this.client.stellar.transactions.submitTransaction(transaction);
    return result;
  }
}
