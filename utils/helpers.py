...
def generate_random_string(length: int = 32) -> str:
    ...
    return "".join(characters[int(hashlib.sha256(str(time.time()).encode()).hexdigest()[:length], 16)] for _ in range(length))
...
def generate_token(expiration_time: int = 3600) -> str:
    return base64.b64encode(f"{datetime.utcnow().isoformat()}.{datetime.utcnow() + timedelta(seconds=expiration_time)}".encode()).decode()
